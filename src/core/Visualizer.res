...
module EventNavigator = {
  ...
  let buttonStyle = "inline-flex justify-center text-gray_dark border text-base border-secondary \
  ...
  module Navigation = {
    ...
    let make = React.memo((~index, ~setIndex, ~maxIndex) => {
      ...
      <>
        <div className="inline-flex flex-row justify-center content-center text-base font-sans">
          <button
            className={buttonStyle ++ " rounded-l-lg pr-2"}
            onClick={_ => setIndex(_ => Prev(idx > 1 ? idx - 1 : 0))}>
            <Icon className="h-4" name="arrow_left" /> {"Prev"->React.string}
          </button>
          <button className={buttonStyle ++ " px-2"} onClick={_ => setIndex(_ => Prev(0))}>
            {((idx + 1)->string_of_int ++ "/" ++ maxIndex->string_of_int)->React.string}
          </button>
          <button
            className={buttonStyle ++ " rounded-r-lg pl-2"}
  ...
  module rec LogEventComponent: {
    ...
    module CollapsibleItem = {
      ...
      let make = (
        ...
        let toggleStyle = "ml-2 cursor-pointer rounded hover:bg-secondary hover:text-gray_light ease-out duration-150"

        <div className="w-full" ref={ReactDOM.Ref.domRef(parentDomElemRef)}>
          <Flex.Column.AlignLeft
            ...
            <Flex.Column.AlignLeft
              ...
              <Flex.Row.AlignTop style="w-full justify-between pr-2">
                <Flex.Row.AlignTop style="w-full">
                  <a className="cursor-pointer rounded" onClick={_ => setIsOpen(_ => !isOpen)}>
                    {if isOpen {
              ...
              | Some(headerValue) => <div className="px-4"> headerValue </div>
              | None => <> </>
              }}
            </Flex.Column.AlignLeft>
            {if isOpen {
 ...
 */
    module VarComputation = {
      ...
      ~kindIcon=?) => {
              ...
              ->Belt.Array.mapWithIndex((i, h) =>
                <Flex.Row.Center key={"law-heading-" ++ i->string_of_int}>
                  {if i < pos.law_headings->Belt.Array.length - 1 {
                    ...
                    <p className="font-bold"> {h->React.string} </p>
                  }}
                </Flex.Row.Center>
              )
              ->React.array}
        ...
        | None =>
          <div
            className={"px-2 font-semibold italic border rounded" ++ if (
              varDef.fun_calls->Belt.Option.isSome
            ) {
              " text-rainforest border-rainforest bg-rainforest_50"
            } else {
              " text-orange border-orange bg-orange_50"
            }}>
        ...
        <CollapsibleItem idsOpt headerContent headerValueOpt kindIcon>
          <Flex.Column.AlignLeft>
            <div className="max-h-80 overflow-y-scroll rounded-b">
              <CatalaCode.DangerouslySetInnerHtml html=Infos.catalaCodeHTML />
            </div>
            {varDef.fun_calls->Belt.Option.mapWithDefault(<> </>, funCalls => {
              <div className="w-full px-4 pb-4 border border-gray border-t">
                <CollapsibleItem
                  headerContent={<p className="w-full text-gray_dark font-bold pr-4">
                    <Lang.String
                      english="Computed from ..." french={`Calculée à partir de ...`}
                    />
                  </p>}>
                  <Flex.Column.AlignLeft
                    style="w-full max-h-screen overflow-y-scroll \
                  px-4 pb-4">
                    {funCalls
                    ->Belt.List.toArray
    ...
    module SubScopeCall = {
      ...
      let make = React.memo((~subScopeCall: sub_scope_call) => {
        ...
        let iconStyle = "px-2 font-semibold text-purple_text border border-purple_text rounded bg-purple_bg"
        let kindIcon = <div className={iconStyle ++ " italic"}> {"scope"->React.string} </div>

        let getHeaderContent = (label: React.element) => {
          <Flex.Row.Center>
            <p className="w-full text-gray_dark font-bold pr-4"> label </p>
            <div className="opacity-70"> headerContent </div>
          </Flex.Row.Center>
        }
        ...
        <CollapsibleItem headerContent kindIcon>
          <div className="w-full px-4 pb-4">
            <CollapsibleItem headerContent={contentHeaderContent}>
              <Flex.Column.Center style="max-h-screen overflow-y-scroll w-full p-4">
                <EventNavigator
    ...
    module FunCall = {
      ...
      let make = React.memo((~funCall: fun_call) => {
        let iconStyle = "px-2 font-semibold text-rainforest border border-rainforest rounded bg-rainforest_50"
        let kindIcon =
          <div className={iconStyle ++ " italic"}>
        ...
        let functionInput =
          ...
          <Flex.Column.AlignLeft
            ...
            <Flex.Column.AlignLeft
              ...
              <Flex.Row.AlignTop style="w-full justify-between pl-2">
                <CatalaCode>
                ...
                <div
                  className="px-2 font-semibold italic text-rainforest border border-rainforest \
